...
find_gender = grep(pattern = 'sexData', x = list.files(path = folder_outputs))
find_location = grep(pattern = 'locationsData', x = list.files(path = folder_outputs))
save_total = list()
save_length = list()
save_gender = list()
save_locations = list()
for(i in seq_along(find_total)) { 
  ...
  save_gender[[i]] = read.csv(file.path(folder_outputs, list.files(path = folder_outputs)[find_gender[i]]))
  save_locations[[i]] = read.csv(file.path(folder_outputs, list.files(path = folder_outputs)[find_location[i]]))
}
...
data_gender = dplyr::bind_rows(save_gender)
data_gender$precision = factor(data_gender$precision, levels = precisionScenarios$name)
data_locations = dplyr::bind_rows(save_locations)
...
# Plot 2: -----------------------------------------------------------------
ggplot(data_gender, aes(x=factor(nObservers), y = bias)) +
  geom_boxplot(aes(fill = factor(n_sample), color = factor(n_sample)), alpha = 0.2, outlier.size = 0.5) +
  xlab('Number of observers') +
  ylab('RE') +
  labs(col = "Number fish sampled", fill = "Number fish sampled") +
  ...
  scale_fill_brewer(palette = 'Spectral') +
  scale_color_brewer(palette = 'Spectral') +
  theme(legend.position = c(0.2, 0.75), legend.background = element_rect(fill = "transparent"),
        legend.title = element_text(size = 8), 
        legend.text = element_text(size = 8),
        legend.key.size = unit(0.5, 'cm'))  +
  facet_wrap(. ~ precision)
ggsave(filename = file.path(folder_figures, 'bias_gender_nObs.png'),
       width = 190, height = 90, units = 'mm', dpi = 500)


# Plot 3 ------------------------------------------------------------------

for(k in seq_along(nObservers)) {
  ...
  # Plot 2b: Bias per length and sex
  tmp_data = data_length %>% 
    dplyr::filter(nObservers == k) %>% 
    dplyr::group_by(fish_lengths, precision, n_sample, gender2) %>% 
