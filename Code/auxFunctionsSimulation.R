...
getnDaysTrip = function(nDaysFSseason){

  saveDays = NULL

  while(sum(saveDays) < nDaysFSseason){

    tmp_day = sample(x = minDay:maxDay, size = 1)
    saveDays = c(saveDays, tmp_day)

  }

  saveDays = saveDays[-length(saveDays)]
  saveDays = c(saveDays, nDaysFSseason-sum(saveDays))
